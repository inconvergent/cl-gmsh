...
(veq:fvdef load-scene (size)
  ...
  (let* ((sc (gmsh/scene:scene/make :s (/ 1200f0) :xy (veq:f2$zero)))
    ...
    (macrolet ((box (&body body) `(gmsh/io:obj/load-model :cube :msh msh ,@body))
      ...
      (setmat (:ro :y) (box :xy (veq:f3$point s- 0f0 0f0) :s (veq:f3$val ss)))
      (setmat (:ro :c) (box :xy (veq:f3$point 0f0 s- 0f0 ) :s (veq:f3$val ss)))
      (setmat (:ro :m) (box :xy (veq:f3$point 0f0 0f0 s-) :s (veq:f3$val ss)))
