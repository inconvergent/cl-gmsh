...
" (render-wrap ; NOTE: requires scene: sc ; TODO: rewrite/make this explict
    (labels
      ((do-ao (depth hi (:va 3 rgb pt dir)) ; ambient occlusion
        ...
        (veq:xlet ((f3!hn (get-normal bvh hi dir))
          ...
          (f3!@+ (f3!@.* 0.7 (f3!@+ (f3!@*. rgb (* 0.8 ao))
                 ...
                 (f3!@*. (f3!@* rgb (m@do-render (1+ depth) hp*
                                      (m@reflect dir
                                        (veq:f3norm (f3!@+ (srnd:3in-sphere *rs* 0.1) hn)))))
                         0.4))))

       (do-cr (depth hi (:va 3 rgb pt dir)) ; ao + reflection
        (declare (veq:pn depth) (veq:in hi) (veq:ff rgb pt dir))
        (veq:xlet ((f3!hn (get-normal bvh hi dir))
                   (f3!hp* (veq:f3from pt hn 0.001)))
             ...
             (veq:mvb (flag (:va 3 rgb)) (hitmat bvh hi)
               ...
               (veq:xlet
                 ((f3!pt (veq:f3from p ll hs))
                  (f3!res (case flag
                            (:ao (m@do-ao depth hi rgb pt dir))
                 ...
                 (if (and vol (< depth vdepth))
                   (f3!@+ res (f3!@.* vmult
                                (m@ll-sample-volume bvh vrec vlim vexpt vdst hi p
                                  (f3!@+ p (f3!@*. ll hs)) ;
                                  (veq:f3norm (f3!@+ dir (srnd:3in-sphere *rs* 0.05))))))
                   (veq:f3 res)))))))
