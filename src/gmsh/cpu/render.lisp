...
(veq:fvdef ll-sample-volume (bvh maxdepth lim vexpt hi (:va 3 origin pt d)) ;pt = hit
   (declare #.*opt1* (fixnum hi maxdepth) (veq:ff origin pt d lim vexpt))
...
(veq:vdef render (sc bvh &key (aa 1) (par t) (size 1000) (bs 1) (raylen 2000.0)
                              vol (vmult 134.0)  (vexpt 0.3)
             ...
             (veq:mvb (flag (:va 3 rgb)) (hitmat bvh hi)
               ...
               (veq:xlet ((f3!pt (veq:f3from p ll hs))
                ...
                (if (and vol (< depth vdepth))
                  (f3!@+ res (f3!@.* vmult
                               (m@ll-sample-volume bvh vrec vlim vexpt hi p
                                 (f3!@+ p (f3!@*. ll hs)) ;
