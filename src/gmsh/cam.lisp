...
(veq:fvdef -prt (o s)
  (labels ((fmtvec (name s (:va 3 v)) (lqn:fmt "~a:~a~08,2f | ~08,2f | ~08,2f" name s v)))
    (format s "<@cam (~a ~a)~%  ~a~%  ~a~%  ~a
              ...
              (m@fmtvec :up "   " (@up o))
              (m@fmtvec :vpn "  " (@vpn o))
              (m@fmtvec :orth " " (@par o 0))
              (m@fmtvec :proj " " (@par o 1)))))
...
(defstruct (cam (:constructor make) (:print-object -prt))
  ...
  (pos       (veq:f3$pt 150.0 150.0 50.0)    :type veq:3fvec :read-only t)  ; position
  (up        (veq:f3$pt 0.0 1.0 0.0)         :type veq:3fvec :read-only t)  ; up
  (vpn       (veq:f3$pt 0.0 0.0 1.0)         :type veq:3fvec :read-only t)  ; away from dop
  (par       (veq:f$~ (6) 1000.0 0.1 1000.0                                 ; ortho: s near far
                             0.1 0.1 1000.0) :type veq:fvec  :read-only t)  ; proj:  s near far
  (u         (veq:f3$val 0.0)                :type veq:3fvec :read-only t)  ; cam horizontal
  (v         (veq:f3$val 0.0)                :type veq:3fvec :read-only t)) ; vertical
...
(defun set-nav-mode (c mode)
  (declare #.*opt* (cam c) (keyword mode)) "set nav mode see: *proj-modes*"
  (setf (cam-nav-mode c) mode))
(defun roll-nav-mode (c)
  (declare #.*opt* (cam c)) "next nav-mode. see *nav-modes*"
  (set-nav-mode c (nxt-mode *nav-modes* (@nav-mode c))))

(defun set-proj-mode (c mode)
  (declare #.*opt* (cam c) (keyword mode)) "set proj mode. see: *proj-modes*"
  (setf (cam-proj-mode c) mode))
(defun roll-proj-mode (c)
  (declare #.*opt* (cam c)) "next proj mode. see *proj-modes*"
  (set-proj-mode c (nxt-mode *proj-modes* (@proj-mode c))))

(veq:fvdef vm (c) (declare #.*opt1* (cam c))
  "view matrix, compatible with gmsh/scene and gmsh/xrend."
  (veq::m@fmake-view-matrix (@pos c) (f3!@- (@pos c) (@vpn c)) (@up c)))
(veq:fvdef pm (c &optional (mode (@proj-mode c))) (declare #.*opt1* (cam c) (keyword mode))
...
(veq:fvdef scale-from-to! (c from to)
  ...
  (veq:xlet ((f!r (veq:ff (/ to from))))
    (setf (@par c 0) (f3!@*. (@par c 0) r)
          (@par c 1) (f3!@*. (@par c 1) r)))
  c)
(defun proj-par (c &optional (mode (@proj-mode c)))
  (declare #.*opt1* (cam c) (keyword mode))
...
(defun nav/s! (c val &optional (mode (@proj-mode c)))
  (declare #.*opt1* (cam c) (veq:ff val) (keyword mode))
...
(defun nav/axis! (c axis &optional (stp 1.0) (trans 16.0) (yaw 0.03)
                                             (pitch 0.03) (roll 0.1))
  (declare #.*opt1* (cam c) (veq:fvec axis) (veq:ff stp trans yaw pitch roll))
...
(veq:fvdef make-uv ((:va 3 up vpn)) (declare #.*opt1* (veq:ff up vpn))
...
(veq:fvdef look-vpn (pos look) (declare #.*opt1* (veq:3fvec pos look))
  (veq:f3norm (f3!@- (veq:f3$ pos) (veq:f3$ look))))

(veq:fvdef update! (c &key up pos vpn look) (declare #.*opt1* (cam c))
...
from dop); or look to set view plane normal relative to position."
  ...
  (when pos  (setf (@pos c) (veq:f3$ pos)))
  (when up   (setf (@up  c) (veq:f3$ up)))
  (when vpn  (setf (@vpn c) (veq:f3$ vpn)))
  (when look (setf (@vpn c) (look-vpn (cam-pos c) look)))
  (when (or pos up vpn look)
        (veq:xlet ((f6!uv (m@make-uv (@up c) (@vpn c))))
          (setf (@u c) (veq:f3 (:vr uv 0 1 2))
                (@v c) (veq:f3 (:vr uv 3 4 5)))))
  c)

(veq:fvdef nav/trans! (c ax val)
  (declare #.*opt1* (cam c) (keyword ax) (veq:ff val)) "translate cam."
  (update! c :pos (veq:f3$pt (f3!@+ (@pos c)
                               (f3!@.* val (ecase ax (:u (@u c)) (:v (@v c))
                                                     (:vpn (@vpn c))))))))

(veq:fvdef nav/around! (c ax val)
  (declare #.*opt1* (cam c) (keyword ax) (veq:ff val)) "rotate around cam axis."
  (unless (> (abs val) 0.0) (return-from nav/around! nil))
...
(defun export-data (c) (declare (cam c)) "serialize cam. see import-data."
